...
export async function proxy(request: NextRequest) {
  ...
  if (user && !isApiRoute) {
    ...
    // live under /institution, team admins everywhere else.
    const role = (user.app_metadata?.role as string) ?? "admin";
    const wsType = (user.app_metadata?.workspace_type as string) ?? "team";
    ...
    if (role === "member") {
      ...
      // Team admin — keep out of member + institution areas.
